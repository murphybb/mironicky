...
class RequestStatusService:
    ...
    def __init__(self):
        ...
        # Use a low-timeout dedicated client profile for request status writes.
        self._redis_client_name = "request_status"
        self._redis_socket_timeout = float(
            os.getenv("REQUEST_STATUS_REDIS_SOCKET_TIMEOUT", "0.5")
        )
        self._redis_socket_connect_timeout = float(
            os.getenv("REQUEST_STATUS_REDIS_SOCKET_CONNECT_TIMEOUT", "0.3")
        )
        self._redis_failure_backoff_seconds = float(
            os.getenv("REQUEST_STATUS_REDIS_FAILURE_BACKOFF_SECONDS", "10")
        )
        self._redis_retry_after_monotonic = 0.0
    ...
    def _is_redis_in_cooldown(self) -> bool:
        return time.monotonic() < self._redis_retry_after_monotonic

    def _mark_redis_failure(self) -> None:
        self._redis_retry_after_monotonic = (
            time.monotonic() + self._redis_failure_backoff_seconds
        )

    async def _get_request_status_client(self):
        redis_provider = self._get_redis_provider()
        return await redis_provider.get_named_client(
            self._redis_client_name,
            socket_timeout=self._redis_socket_timeout,
            socket_connect_timeout=self._redis_socket_connect_timeout,
        )
    ...
    async def update_request_status(
        ...
        try:
            if self._is_redis_in_cooldown():
                logger.debug(
                    "Skip request status update during Redis cooldown: tenant_key_prefix=%s, req=%s",
                    tenant_info.tenant_key_prefix,
                    request_id,
                )
                return False
            client = await self._get_request_status_client()
    ...
    async def get_request_status(
        ...
        try:
            if self._is_redis_in_cooldown():
                logger.debug(
                    "Skip request status read during Redis cooldown: tenant_key_prefix=%s, req=%s",
                    tenant_info.tenant_key_prefix,
                    request_id,
                )
                return None
    ...
    async def delete_request_status(
        ...
        try:
            if self._is_redis_in_cooldown():
                logger.debug(
                    "Skip request status deletion during Redis cooldown: tenant_key_prefix=%s, req=%s",
                    tenant_info.tenant_key_prefix,
                    request_id,
                )
                return False
