...
from core.asynctasks.arq_client import enqueue_job
...
from core.request.timeout_background import is_background_mode_enabled
...
FETCH_MEMORY_TYPES = [
    ...
    MemoryType.FORESIGHT.value,
    MemoryType.EVENT_LOG.value,
]
SEARCH_MEMORY_TYPES = [
    MemoryType.EPISODIC_MEMORY.value,
    MemoryType.FORESIGHT.value,
...
SUPPORTED_RETRIEVE_METHODS = [method.value for method in RetrieveMethod]


def get_current_request_id() -> str | None:
    """Return the request_id propagated by AppLogicMiddleware."""
...
def build_memorize_response(memory_count: int, request_id: str | None) -> Dict[str, Any]:
    """Build the canonical POST /api/v1/memories response envelope."""
    message = (
        f"Extracted {memory_count} memories"
        if memory_count > 0
        else "Message queued, awaiting boundary detection"
    )
    status_info = "extracted" if memory_count > 0 else "accumulated"
    return {
...
def build_background_memorize_response(request_id: str | None) -> Dict[str, Any]:
    """Build the canonical 202 response for worker-backed memorize requests."""
    return {
        "status": ErrorStatus.OK.value,
        "request_id": request_id,
...
def build_memory_types_response() -> Dict[str, Any]:
    """Expose supported fetch/search capabilities."""
    return {
        "status": ErrorStatus.OK.value,
        "message": "Supported memory capabilities retrieved successfully",
        "result": {
            "memory_types": FETCH_MEMORY_TYPES,
            "search_memory_types": SEARCH_MEMORY_TYPES,
            "retrieve_methods": SUPPORTED_RETRIEVE_METHODS,
        },
    }
...
class MemoryController(BaseController):
    ...
    async def memorize_single_message(
        ...
        try:
            ...
            if request_id is None:
                raise RuntimeError("Missing request_id in application context")

            if is_background_mode_enabled(request):
                app_info = get_current_app_info() or {}
                tenant_info = get_bean_by_type(
                    RequestTenantProvider
                ).get_tenant_info_from_request(request)
                request_status_context = {
                    "request_id": request_id,
                    "tenant_key_prefix": tenant_info.tenant_key_prefix,
                    "request_start_time_ms": app_info.get("request_start_time_ms"),
                    "url": str(request.url),
                    "method": request.method,
                }
                await enqueue_job(
                    "memorize_single_message_job",
                    message_data,
                    request_status_context,
                    job_id=request_id,
                )
                logger.info(
                    "Memory request enqueued to worker: request_id=%s, group_id=%s",
                    request_id,
                    group_id,
                )
                record_memorize_request(
                ...
                return JSONResponse(
                    status_code=202,
                    content=build_background_memorize_response(request_id),
                )

            # 4. Call memory_manager to process the request synchronously
            logger.info("Starting to process memory request in sync mode")
            ...
            return build_memorize_response(
    ...
    @get(
        "/memory-types",
        response_model=MemoryTypesResponse,
        summary="Get supported memory types and retrieve methods",
        description="""
        Return the memory types supported by fetch/search endpoints and the
        retrieve methods accepted by the search endpoint.
        """,
    )
    async def get_memory_types(self) -> MemoryTypesResponse:
        """Expose supported fetch/search capabilities for client probes."""
        return build_memory_types_response()
