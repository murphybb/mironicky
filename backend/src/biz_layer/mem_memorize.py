...
async def _trigger_profile_extraction(
    ...
    try:
        ...
        llm_provider = LLMProvider(
            ...
            model=os.getenv("LLM_MODEL", "MiniMax-M2.5"),  # skip-sensitive-check
...
async def _update_memcell_and_cluster(state: ExtractionState):
    ...
    # Trigger clustering in the background so write requests do not block on
    # downstream enrichment work such as profile extraction.
    try:
        ...
        async def _run_clustering_job():
            try:
                ...
                logger.error(
                    f"[MemCell Processing] 鉂?Failed to trigger clustering: {exc}"
                )

        asyncio.create_task(_run_clustering_job())
        logger.info(
            f"[MemCell Processing] ✅ Clustering completed (scene={state.scene})"
        )
    except Exception as e:
