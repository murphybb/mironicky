...
class HybridVectorizeConfig:
    ...
    def __post_init__(self):
        ...
        # SCNet's embedding endpoint currently accepts at most 5 inputs per request.
        # Clamp here so higher generic defaults do not break runtime extraction flows.
        if "api.scnet.cn" in (self.primary_base_url or "") and self.batch_size > 5:
            logger.warning(
                "SCNet embedding batch size limit detected, clamping VECTORIZE_BATCH_SIZE from %s to 5",
                self.batch_size,
            )
            self.batch_size = 5
