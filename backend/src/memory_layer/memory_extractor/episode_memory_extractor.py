    ...
    async def _compute_embedding(self, text: str) -> Optional[dict]:
        ...
        try:
            ...
            timeout_seconds = float(
                os.getenv("EPISODE_EMBEDDING_TIMEOUT_SECONDS", "15")
            )
            vec = await asyncio.wait_for(
                vs.get_embedding(text), timeout=timeout_seconds
            )
            ...
            logger.warning(
                "Episode embedding timed out, continuing without vector payload"
            )
            return None
        except Exception as e:
