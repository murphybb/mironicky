...
def create_cheap_provider() -> OpenAIProvider:
    """Create a cheap provider (using MiniMax-M2.5)"""
    return create_provider(model="MiniMax-M2.5", temperature=0.3, max_tokens=1024)


def create_high_quality_provider() -> OpenAIProvider:
    """Create a high-quality provider (using MiniMax-M2.5)"""
    return create_provider(model="MiniMax-M2.5", temperature=0.7, max_tokens=4096)
