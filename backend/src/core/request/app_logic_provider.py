...
class AppLogicProvider(ABC):
    ...
    async def on_request_complete(
        ...
        app_info_override: Optional[Dict[str, Any]] = None,
    ) -> None:
        """
        ...
        Args:
            ...
            app_info_override: Explicit app_info payload for background callbacks
        """
        # Default implementation is empty; subclasses may optionally override
        _ = (request, http_code, error_message, app_info_override)
...
class AppLogicProviderImpl(AppLogicProvider):
    ...
    def __init__(self):
        self._request_status_service: Optional[RequestStatusService] = None
        self._request_tenant_provider: Optional[RequestTenantProvider] = None

    def _get_request_status_service(self) -> RequestStatusService:
        if self._request_status_service is None:
            self._request_status_service = get_bean_by_type(RequestStatusService)
        return self._request_status_service
    ...
    def setup_app_context(self, request: Request) -> Dict[str, Any]:
        ...
        app_info['request_start_time_ms'] = int(time.time() * 1000)

        return app_info

    async def on_request_begin(self, request: Request) -> None:
        app_info = self.get_current_app_info() or {}
        request_id = app_info.get("request_id")
        if not request_id:
            return

        tenant_info = self._get_request_tenant_provider().get_tenant_info_from_request(
            request
        )
        start_time_ms = app_info.get("request_start_time_ms")
        await self._get_request_status_service().update_request_status(
    ...
    async def on_request_complete(
        ...
        if http_code == 202:
            # Background mode keeps the request in "start" until the async task finishes.
            return

        app_info = app_info_override or self.get_current_app_info() or {}
        request_id = app_info.get("request_id")
        if not request_id:
            return
        ...
        if isinstance(start_time_ms, int):
            time_ms = max(end_time_ms - start_time_ms, 0)

        status = "success" if http_code < 400 else "failed"
        await self._get_request_status_service().update_request_status(
            tenant_info=tenant_info,
            request_id=request_id,
            status=status,
