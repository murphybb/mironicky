...
class _WindowsSafeStreamHandler(logging.StreamHandler):
    """Windows console-safe stream handler.

    Prevents UnicodeEncodeError from bubbling into logging error traces when the
    active console code page cannot represent some characters (e.g. emoji).
    """
...
class LoggerProvider:
    ...
    def _setup_root_logging(self):
        ...
        if os.name == 'nt':
            stream_handler = _WindowsSafeStreamHandler(sys.stdout)
        else:
