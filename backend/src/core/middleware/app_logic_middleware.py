...
_client_disconnect_total = Counter(
    name="http_client_disconnect_total",
    documentation="Total number of client disconnects while request body is read",
...
class AppLogicMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            logger.warning(
                "Client disconnected while reading request body: method=%s path=%s",
                request.method,
                request.url.path,
            )
            _client_disconnect_total.labels(
                method=request.method, path=request.url.path
            ).inc()
            return JSONResponse(
                status_code=499,
                content={
                    "error_code": "research.client_disconnected",
