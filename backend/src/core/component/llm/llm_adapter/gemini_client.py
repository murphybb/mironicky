...
try:
    ...
    ContentDict = Any  # type: ignore[assignment]
    GenerateContentConfig = Any  # type: ignore[assignment]
    ThinkingConfig = Any  # type: ignore[assignment]
    GOOGLE_GENAI_IMPORT_ERROR = exc


def require_google_genai() -> None:
    if GOOGLE_GENAI_IMPORT_ERROR is not None:
        raise RuntimeError(
            "google-genai is required only when Gemini client paths are used. "
            "Install the 'google-genai' package to enable Gemini backends."
        ) from GOOGLE_GENAI_IMPORT_ERROR
