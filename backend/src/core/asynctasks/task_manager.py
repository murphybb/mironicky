...
try:
    ...
    class Job:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs):
            raise RuntimeError("arq is required to use async task jobs")

    class Worker:  # type: ignore[no-redef]
        pass

    class Function:  # type: ignore[no-redef]
        def __init__(self, coroutine=None, name=None, max_tries=None, timeout=None):
            self.coroutine = coroutine
            self.name = name
            self.max_tries = max_tries
            self.timeout = timeout

    def arq_func(coroutine, name=None, max_tries=None, timeout=None):  # type: ignore[no-redef]
        return Function(
            coroutine=coroutine,
            name=name,
            max_tries=max_tries,
...
class TaskManager:
    ...
    def _require_arq(self) -> None:
        if create_pool is None:
            raise RuntimeError(
                "arq is required to use async task execution. "
