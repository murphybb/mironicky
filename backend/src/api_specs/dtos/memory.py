...
class MemorizeResponse(BaseApiResponse[MemorizeResult]):
    ...
    request_id: Optional[str] = Field(
        default=None,
        description="Traceable request identifier for status polling",
        examples=["req_123456"],
    )
...
# =============================================================================
# Memory Capabilities DTOs (GET /api/v1/memories/memory-types)
# =============================================================================


class MemoryTypesResult(BaseModel):
    """Supported memory/search capabilities exposed by the API."""

    memory_types: List[MemoryType] = Field(
        default_factory=list,
        description="Memory types supported by fetch/list endpoints",
    )
    search_memory_types: List[MemoryType] = Field(
        default_factory=list,
        description="Memory types supported by the search endpoint",
    )
    retrieve_methods: List[RetrieveMethod] = Field(
        default_factory=list,
        description="Retrieve methods accepted by the search endpoint",
    )
...
class MemoryTypesResponse(BaseApiResponse[MemoryTypesResult]):
    """Memory capability response for GET /api/v1/memories/memory-types."""

    result: MemoryTypesResult = Field(
        default_factory=MemoryTypesResult,
        description="Supported memory types and retrieve methods",
    )
